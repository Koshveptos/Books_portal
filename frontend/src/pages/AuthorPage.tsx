...
import { authorService, bookService } from '../services/api';
...
const AuthorPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAuthorData = async () => {
      try {
        setLoading(true);
        const authorData = await authorService.getAuthor(Number(id));
        setAuthor(authorData);

        const booksData = await bookService.getBooksByAuthor(Number(id));
  ...
  if (error || !author) {
    return (
      <div className="flex justify-center items-center min-h-screen">
        <div className="text-red-500 text-xl">{error || 'Автор не найден'}</div>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
        ...
        {author.birth_date && (
          <p className="text-gray-600 mb-2">
            Дата рождения: {new Date(author.birth_date).toLocaleDateString()}
          </p>
        )}
      </div>

      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold">Книги автора</h2>
