...
from fastapi_users.password import PasswordHelper  # E402
from sqlalchemy import select
from sqlalchemy.exc import IntegrityError
from sqlalchemy.ext.asyncio import AsyncSession  # E402

from app.core.database import AsyncSessionLocal, engine  # E402
from app.models import Author, Book, Category, Tag, User  # E402
...
async def create_or_get_author(db: AsyncSession, author_data: dict) -> Author:
    try:
        ...
        print(
            f"Ошибка при создании автора {author_data.get('name', 'Unknown')}: {str(e)}"
        )
        await db.rollback()
        return None
...
async def create_or_get_category(db: AsyncSession, category_data: dict) -> Category:
    try:
        ...
        stmt = select(Category).where(
            Category.name_categories == category_data["name_categories"]
        )
        result = await db.execute(stmt)
        ...
        print(
            f"Ошибка при создании категории {category_data.get('name_categories', 'Unknown')}: {str(e)}"
        )
        await db.rollback()
        return None
...
async def create_or_get_tag(db: AsyncSession, tag_data: dict) -> Tag:
    try:
        ...
        print(
            f"Ошибка при создании тега {tag_data.get('name_tag', 'Unknown')}: {str(e)}"
        )
        await db.rollback()
        return None
...
async def seed_database():
    ...
    if not data_file.exists():
        ...
        print(
            "Убедитесь, что файл seed_data.json находится в директории backend/app/scripts/data/"
        )
        return
        ...
        async with AsyncSessionLocal() as db:
            ...
            for book_data in data.get("books", []):
                try:
                    ...
                    missing_fields = [
                        field for field in required_fields if field not in book_data
                    ]
                    if missing_fields:
                        print(
                            f"Пропущена книга: отсутствуют поля {', '.join(missing_fields)}"
                        )
                        continue
                    ...
                    print(
                        f"Ошибка при создании книги '{book_data.get('title', 'Unknown')}': {str(e)}"
                    )
                    await db.rollback()
